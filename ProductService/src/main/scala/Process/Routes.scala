...
import Impl.ApproveProductMessagePlanner
import Impl.SubmitProductForApprovalMessagePlanner
import Impl.RejectProductMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "ApproveProductMessage" =>
        IO(
          decode[ApproveProductMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for ApproveProductMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "SubmitProductForApprovalMessage" =>
        IO(
          decode[SubmitProductForApprovalMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for SubmitProductForApprovalMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "RejectProductMessage" =>
        IO(
          decode[RejectProductMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for RejectProductMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
