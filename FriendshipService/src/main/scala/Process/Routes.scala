...
import Impl.DeleteFriendMessagePlanner
import Impl.DeleteFriendsByUserIDMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "DeleteFriendMessage" =>
        IO(
          decode[DeleteFriendMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for DeleteFriendMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "DeleteFriendsByUserIDMessage" =>
        IO(
          decode[DeleteFriendsByUserIDMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for DeleteFriendsByUserIDMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
