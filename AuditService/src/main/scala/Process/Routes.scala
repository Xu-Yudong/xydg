...
import Impl.RecordActionLogMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "RecordActionLogMessage" =>
        IO(
          decode[RecordActionLogMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for RecordActionLogMessage[${err.getMessage}]")
